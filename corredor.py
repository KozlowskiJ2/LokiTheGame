...
def corredor(mochila):
    ...
    if acao == "1":
        ...
        if resp == "1":
            perder('A porta tinha um sensor que acabou sendo ativado quando você a forçou e alertou todos os guardas...mais sorte da próxima vez.')
        elif resp == "2":
            chave(mochila)
        else:
            perder("Siga as regras do jogo!")
    
    elif acao == "3":
        perder("Você foi procurar algo na cela e acabou esbarrando em um botão de alarme! Mais sorte da próxima vez!")
    else:
        perder("Sem trapacear!")
        corredor(mochila)
    salvar(1)
    salao(mochila)
