...
def briga():
    ...
    while vidasAdversario != 0 or vidasLoki != 0:
        ...
        if acao == 1 and [mochila(i) for i != 1]: #quando tiver arma, estar na posição 1
            usar = abreMochila(mochila)
            if usar == "podador":
                vidasAdversario = 0
            elif usar == "clava":
                vidasAdversario -= 2
                input("Quase um homerun...")
            vidasLoki -= 1
            [input(i) for i in range(2,4)]
            if acao == 2:
            ...
            if soco % 3 == 0:
                ...
                vidasLoki -= 1
                input("Esse você errou. Vai continuar tentando?")
        eliffugir = int(input("Deseja realmente fugir?\nPressione 1 para confirmar ou outra tecla para negar"))
            if fugir == 1:
                perder("Você foi pego. Parece que voce vai ter que pensar em outro plano fugir.")
            else:
                vidasLoki -= 1
                print("Você se distraiu querendo fugir e acabou levando um soco!")                print("Você se distraiu querendo fugir e acabou levando um soco!")
        elif acao == 4:
            input("Pressione Enter para usar os poderes de trapaça!")
            poder = random.randrange(1, 101)
            if poder % 2 == 0:
